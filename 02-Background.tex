...
\begin{figure}[p]
	\centering
	\includegraphics[width=0.7\textwidth]{AlignmentExplanation}
...
Examining the geographic location of the work resulted in a couple of interesting observations. Firstly the high number of papers from Australiasia, and the relatively small number from Northern America. 
...
A clear message from the analysed papers was the value of using multiple data sources in evaluating outcomes. Student satisfaction surveys and unit results appear to be readily available sources of information, and provide valuable evidence of learning outcomes and student perceptions. This can then be further enhanced by analysis of student work and staff reflections. Ethical implications of using student work in research will require informed consent, must address the power imbalance of the teacher-student relationship, and avoid any perception of coercion.

% subsubsection effective_evaluation_sources (end)

\subsubsection{Opportinities for Misalignment} % (fold)
\label{ssub:opportinities_for_misalignment}

There appears to be significant opportunities for misalignment due to te lack of student engagement with the alignment process.

\todo{Draw a picture of ILO -> staff -> AT -> students vs ILO -> students}
...
In many ways the papers analysed indicated a ``business as usual'' approach. Constructive alignment involved staff thinking about how what they asked students to do related to the intended learning outcomes. Ideas of constructivism have become synonymous with either group work, and its associated group assessment, or attempts to make lectures and other classes more interactive. The ideas of learning through mistakes, and centring activities on students appears to have received little attention. Similarly, approaches to assessment have remained primarily unchanged from the standard assignments and a final exam. 

While the approaches taken can be argued to be ``Constructive Alignment'', they do not appear to have captured the dramatic shift in thinking that is reported by \citet{Biggs:1997}. The student centred approach of the portfolios reported by Biggs had alignment with the intended learning outcomes at their heart, perhaps the work reported by Biggs goes beyond basic ``Constructive Alignment'' to capture additional principles that are beneficial to student learning.
